...
`;
let injected=false;
function ensure(){ if(!injected && typeof document!=='undefined'){ const s=document.createElement('style'); s.setAttribute('data-tj','avatar'); s.textContent=CSS; document.head.appendChild(s); injected=true; } }
...
export function Avatar({ src, alt = '', initials, size = 'md', piece = 'i', round = false, className = '', style = {}, ...rest }) {
  ensure();
  const px = typeof size === 'number' ? size : (SIZES[size] || SIZES.md);
  const cls = ['tj-avatar', round ? 'tj-avatar--round' : '', className].filter(Boolean).join(' ');
  return (
    <span className={cls} style={{ width: px, height: px, fontSize: Math.round(px * 0.34), '--av-c': `var(${PIECE[piece] || PIECE.i})`, ...style }} {...rest}>
