...
export function ProgressBar({ value = 0, cells = 10, piece = 's', label, showValue = true, cellHeight = 16, className = '', style = {}, ...rest }: ProgressBarProps) {
  ensure()
  const pct = Math.max(0, Math.min(100, value))
  const filled = Math.round((pct / 100) * cells)
  return (
    <div className={['tj-progress', className].filter(Boolean).join(' ')} style={{ '--prog-c': `var(${PIECE[piece] || PIECE.s})`, ...style } as React.CSSProperties} {...rest}>
      <div className="tj-progress__track" style={{ '--cell-h': `${cellHeight}px` } as React.CSSProperties}>
