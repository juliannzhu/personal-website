...
const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    title: 'TRULYHER',
    piece: 'o',
    tagline: 'Full-stack RAG application helping women in computer science manage imposter syndrome, pairing semantic search across journal entries with a DistilBERT classifier that runs in the browser.',
    tags: ['React', 'Node.js', 'PostgreSQL', 'pgvector', 'PyTorch', 'ONNX'],
    cat: 'ai',
    year: 'Sep 2025 - Aug 2026',
    github: 'https://github.com/ErinGu0/TrulyHer',
    devpost: 'https://devpost.com/software/trulyher',
    writeup: [
      'When you\'re surrounded by talented people, it\'s hard not to compare yourself. For a lot of women in tech, that comparison settles into imposter syndrome, which our team has experienced firsthand. TrulyHer lets you journal by voice or text, then finds the patterns in what you wrote. Semantic search brings back the entries where you felt this way before, and the dashboard turns a pile of them into emotional trend reports across five views instead of a wall of text.',
      'The search runs on PostgreSQL with pgvector, using HNSW vector embeddings so it matches on what an entry means rather than which words it happens to share. For the detection I trained a DistilBERT classifier on 200 hand-labelled entries, reaching 0.92 F1, then calibrated its confidence scores and exported it to ONNX with INT8 quantization so it runs in the browser rather than round-tripping to a server.',
  ...
  },
  {
    ...
    writeup: [
      'GeoMap was a client project for the IB Geography teacher at my high school, who wanted his students to revise case studies somewhere more engaging than a textbook. Geomap presents a world map you can click into, where each country opens a draggable case study window, and a legend filters the map down to whichever unit you are studying. Working for a real client meant the requirements adapted as we worked, which taught us to pivot our strategies when needed.',
      'I was primary on UI design, so I designed the logo, the legend, and the title screen. However, the problem that took longest was hovering. Countries made of many separate regions, like Canada and Indonesia, only changed colour under the cursor, which made the map look broken. Each region was its own SVG path, so I wrote a Python script that read the map source and merged every country down to a single path element. Afterwards, hovering lit the whole country at once. It was the first time I wrote a program whose only job was to fix another program\'s data.',
    ],
    images: [
  ...
  },
  {
    ...
    year: 'Jul 2024',
    writeup: [
      'The theme statement for Shad 2024 was finding ways to integrate green energy into the everyday lives of Canadians, which is broad enough that deciding what to build took far longer than building it. We wanted something nobody would have to change their habits for. Cars already throw energy away constantly: every bump in the road moves the suspension, and that motion turns into heat and disappears. Charg-E uses an electromagnetic vibrational energy harvester that sits in that system and converts that movement into usable power instead.',
      'We drafted the whole design ourselves and 3D printed the prototype, a block that mounts onto the shock absorbers in the suspension. The brainstorming phase was easily the hardest part, since we kept scrapping ideas for not being distinct enough from what already existed.',
    ],
    images: [
